/* ============================================================
   ...
   Two modes:
   • `thumb` set  → render that image (licensed provider art, or
     artwork the client's designer supplies). Drop files into
     public/games/ and set `thumb` in lib/catalogue.ts.
   • no `thumb`   → build a layered SVG from the game id, so every
     tile is distinct, deterministic and ours.

   Provider artwork is never bundled here — it has to come from the
   provider's own CDN under licence.
   ============================================================ */
  ...
  if (thumb) {
    return (
      <div className="game__art game__art--img">
        {/* contain, not cover: this artwork carries the game's name inside the
            image, and a cover-crop silently shaves the lettering off whenever
            the source ratio is not exactly the tile ratio */}
        <Image src={thumb} alt={name} fill sizes="33vw" style={{ objectFit: 'contain' }} />
      </div>
    );
  }
